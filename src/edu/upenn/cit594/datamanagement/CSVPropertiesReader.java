...
public class CSVPropertiesReader implements Reader<Properties> {
	...
	/**
	 * Helper method to separate row by commas except for commas in between quotes.
	 * Currently covers all the cases that regex does except for 1
	 * 
	 * @param row
	 * @return
	 */
	public static List<String> parser(String row) {
		...
		for(int i = 0; i < row.length(); i++) {
			...
			//If encountered a quote " add to the buffer until ", is encountered
			if(current == '"') {
				buffer += current;
				while(i < row.length()) {
					i++;
					...
					buffer += current;
					if((current == '"' && i+1 == row.length()) || (current == '"' &&  row.charAt(i+1) == ',')) {
						i++;
						break;
					}
				}
			...
			//once encountered a comma, should add to the list and clear buffer
			else if(current == ',') {
				output.add(buffer);
				buffer = "";
			}
			//if not a comma, continue appending to buffer
			//in the case that we are at the last character, append the remaining buffer
			else {
				buffer += current;
				if(i == row.length()-1) {
					output.add(buffer);
